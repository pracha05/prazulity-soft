	...
	public function addrestarent(){
		...
		if($this->input->post('submit')){
			if($this->form_validation->run('addrestarent')== TRUE){
				...
				$result = $this->My_model->add_record($bpnr_noE);
				if($result){	
  ...
  <div class="es-wrapper-color" style="background-color:#FAFAFA;"> 
   ...
   <table class="es-wrapper" width="100%" cellspacing="0" cellpadding="0" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px;padding:0;Margin:0;width:100%;height:100%;background-repeat:repeat;background-position:center top;"> 
     <tr style="border-collapse:collapse;"> 
      <td valign="top" style="padding:0;Margin:0;"> 
       ...
       echo '<pre>';print_r($message);exit;
						$mail_result = $this->common_model->mail_send_register($this->input->post('email'),$message);
	...
	public function active_link($dataArr)
	{
		...
		$exist_detail = $this->common_model->exist_detail($data_exists);
		
		if($exist_detail == 0)
		{
			$record_details = $this->common_model->active_add_link($email);
			if($record_details)
			{
